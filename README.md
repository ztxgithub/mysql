...
```shell
    ...
    2.OLAP(OnLine Analysis Processing，联机分析处理)
...
```shell
         ...
         存储引擎       事务             锁粒度               主要应用              忌用
          MyISAM      不支持      支持并发插入的表级锁      select insert        读写操作频繁(操作频繁导致锁表，导致数据库查询阻塞)
        MRG_MYISAM    不支持      支持并发插入的表级锁     分档归档，数据仓库       全局查找过多的场景
         innodb        支持        支持MVCC的行级锁         大多数场景(事务处理)          无
         Archive      不支持       行级锁             日志记录，只支持select insert   需要随机读取，更新，删除
         
        
        
        
   说明:MRG_MYISAM是将多个MyISAM表当一个表使用
